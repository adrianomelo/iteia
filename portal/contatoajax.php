<?php
include_once('classes/bo/ContatoBO.php');
$contatobo = new ContatoBO;

try {
	$retorno=$contatobo->enviaContato($_POST);
	if($retorno!=1){ echo '<div class="aviso sucesso">'.utf8_encode('Sua mensagem foi enviada.').'</div>'; } else { echo utf8_encode('<div class="aviso erro">Não foi possível enviar, por favor tente dentro de instantes.</div>'); } 
} catch(Exception $e) {
