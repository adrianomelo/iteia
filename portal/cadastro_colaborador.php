...
include_once('classes/bo/CadastroCulturalColaboradorBO.php');

$cadbo = new CadastroCulturalColaboradorBO;
$exibir_form = true;
...
if ($enviar) {
	try {
		...
		$exibir_form = false;
		$msgtrue = true;
		Header('Location: /cadastrado?t=2');
		die;
	} catch (exception $e) {
...
$titulopagina = 'Cadastro de colaborador';
$js_busca = true;
...
-->
    ...
    <div id="migalhas"><span class="localizador">Você está em:</span> <a href="/" title="Voltar para a página inicial" id="inicio">Início</a> <span class="marcador">&raquo;</span> <a href="/colaboradores.php">Colaboradores</a> <span class="marcador">&raquo;</span> <span class="atual">Cadastro de colaborador</span></div>
    <div id="conteudo">
      <div class="principal">
        <h2 class="midia">Cadastro de colaborador</h2>
        ...
        <form action="/cadastro_colaborador" method="post" id="cadastro">
...
?>
        <p class="caption no-padding-b"><strong>Dados da instituiçao</strong></p>
		<!-- apenas colaborador -->
			<label for="nome">Nome</label><br />
			<input type="text" class="txt<?=$cadbo->verificaErroCampo('nome')?>" name="nome" value="<?=$cadbo->getValorCampo('nome')?>" maxlength="100" onkeyup="contarCaracteres(this, 'cont_nome', 100);" id="name"/>
			<input name="text3" type="text" id="cont_nome" disabled="disabled" class="txt contador" value="<?=100 - strlen($cadbo->getValorCampo('nome'))?>" size="6" /><br />

          <label for="atuacao">Descrição da atuação</label><br />
          <textarea class="slc<?=$cadbo->verificaErroCampo('atuacao')?>" onkeyup="contarCaracteres(this, 'cont_atuacao', 1400);" id="atuacao" name="atuacao" cols="78" rows="7"><?=$cadbo->getValorCampo('atuacao')?></textarea>
		  <input name="text3" type="text" id="cont_atuacao" disabled="disabled" class="txt contador" value="<?=1400 - strlen($cadbo->getValorCampo('nome'))?>" size="6" /><br />

          <label for="endereco-colaborador">Endereço:</label><br />
          <input type="text" class="txt<?=$cadbo->verificaErroCampo('endereco')?>" name="endereco" value="<?=$cadbo->getValorCampo('endereco')?>" id="endereco-colaborador" /><br/>

          <label for="complemento-colaborador">Complemento:</label>
          <em><small>(opcional)</small></em> <br />
          <input type="text" class="txt" name="complemento" value="<?=$cadbo->getValorCampo('complemento')?>" id="complemento-colaborador" /><br/>

          <label for="bairro-colaborador">Bairro:</label><br />
          <input type="text" class="txt<?=$cadbo->verificaErroCampo('bairro')?>" name="bairro" value="<?=$cadbo->getValorCampo('bairro')?>" id="bairro-colaborador" /><br/>
		...
		<div style="display: inline;" id="mostraestado">
		  ...
		  <label for="telefone-colaborador">Telefone:</label>
          <em><small>(Opcional) (Ex. (81) 3222-3333)</small></em> <br />
          <input type="text" class="txt small fone" id="telefone-colaborador" name="telefone" value="<?=$cadbo->getValorCampo('telefone')?>" /><br />

		  <label for="final_endereco">Endereço no iTEIA:</label>
          <em><small>(São permitidos apenas letras (a-z), números (0-9) e o sinal (-). Ex.casa-de-bonecos)</small></em><br />
		   http://www.iteia.org.br/
          <input type="text" class="txt medium<?=$cadbo->verificaErroCampo("finalendereco")?>" id="final_endereco" value="<?=$cadbo->getValorCampo('finalendereco')?>" maxlength="30" name="finalendereco" onkeyup="contarCaracteres(this, 'cont_final_endereco', 30); exibeTexto(this, 'texto_final_endereco'); lowercase('final_endereco');" />
          <input type="text" class="txt contador" disabled="disabled" id="cont_final_endereco" value="<?=30 - strlen($cadbo->getValorCampo('finalendereco'))?>" size="4" />

		  <!-- cadastro autor -->
		  <p class="caption no-padding-b"><strong>Dados do responsável pela solicitação</strong></p>

		<label for="nome_autor">Nome:</label>
		<em><small>(O mesmo do documento de identificação)</small></em><br />
        <input type="text" class="txt<?=$cadbo->verificaErroCampo('nome_autor')?>" name="nome_autor" value="<?=$cadbo->getValorCampo('nome_autor')?>" id="nome_autor" /><br/>

		<label for="artistico_autor">Nome artístico:</label>
		<em><small>(Opcional)</small></em><br />
		<input type="text" class="txt" id="artistico" name="nomeartistico_autor" value="<?=$cadbo->getValorCampo('nomeartistico_autor')?>" maxlength="100" onkeyup="contarCaracteres(this, 'cont_nome_artistico', 100);" />
		<input name="text3" type="text" id="cont_nome_artistico" disabled="disabled" class="txt contador" value="<?=100 - strlen($cadbo->getValorCampo('nomeartistico_autor'))?>" size="6" /><br />

		<label for="historico">Biografia:</label>
		<em><small>(Fale um pouco sobre você)</small></em> <br />
		<textarea id="historico" cols="78" rows="7" class="<?=$cadbo->verificaErroCampo('historico_autor')?>" name="historico_autor" onkeyup="contarCaracteres(this, 'cont_historico', 1400);"><?=$cadbo->getValorCampo('historico_autor')?></textarea>
		<input name="text32" type="text" id="cont_historico" disabled="disabled" class="txt contador" value="<?=1400 - strlen($cadbo->getValorCampo('historico_autor'))?>" size="6" /><br />

        <label for="nascimento">Data de nascimento:</label>
        <em><small>(Opcional)</small></em><br />
        <input type="text" class="txt small date" name="datanascimento_autor" value="<?=$cadbo->getValorCampo('datanascimento_autor')?>" id="nascimento"/><br/>
		...
		<fieldset id="doc">
            ...
            <select id="tipo_doc" name="codtipo_autor" class="slc mini">
				...
				<option value="3" <?=($cadbo->getValorCampo('codtipo_autor') == 3 ? 'selected="selected"' : '');?>>CPF</option>
				<option value="4" <?=($cadbo->getValorCampo('codtipo_autor') == 4 ? 'selected="selected"' : '');?>>Passaporte</option>
				<option value="5" <?=($cadbo->getValorCampo('codtipo_autor') == 5 ? 'selected="selected"' : '');?>>RG</option>
            </select>
          </div>
          <div id="d_numero">
            <label for="numero_doc">Número:</label>
            <br />
            <input type="text" class="txt small" name="numero_autor" value="<?=$cadbo->getValorCampo('numero_autor')?>" id="numero_doc"/>
          </div>
          <div id="d_orgao">
		...
		<input type="text" class="txt<?=$cadbo->verificaErroCampo('email_autor')?>" id="email" name="email_autor" value="<?=$cadbo->getValorCampo('email_autor')?>" /><br/>

		<label for="endereco">Endereço:</label>
		<em><small>(opcional)</small></em> <br />
		<input type="text" class="txt<?=$cadbo->verificaErroCampo('endereco_autor')?>" id="endereco" name="endereco_autor" value="<?=$cadbo->getValorCampo('endereco_autor')?>" /><br />

		<label for="complemento">Complemento: </label>
		<em><small>(opcional)</small></em> <br />
		<input name="complemento_autor" type="text" class="txt" id="complemento" value="<?=$cadbo->getValorCampo('complemento_autor')?>" /><br />

		<label for="bairro">Bairro:</label>
		<em><small>(opcional)</small></em> <br />
		<input type="text" name="bairro_autor" class="txt" id="bairro" value="<?=$cadbo->getValorCampo('bairro_autor')?>" /><br />

        <label for="pais">País:</label><br />
        <select name="codpais_autor" id="pais2" class="slc" onchange="javascript:exibeEstadoCidade2();" <?=$cadbo->verificaErroCampo("pais_autor")?>>
		<?php
		$lista_paises = $cadbo->getListaPaises();
		$pais_selecionado = $cadbo->getValorCampo("codpais_autor");
		if (!$pais_selecionado)
			$pais_selecionado = 2;
		foreach ($lista_paises as $pais) {
			echo "<option value=\"".$pais["cod_pais"]."\"";
		...
		<div style="display: inline;" id="mostraestado2">
			<label for="estado">Estado:</label><br />
			<select name="codestado_autor" id="estado2" onchange="obterCidades2(this, <?=(int)$cadbo->getValorCampo("codcidade_autor")?>)" class="slc<?=$cadbo->verificaErroCampo("estado_autor")?>">
		<?php
		echo "<option value=\"0\"";
		if (!$codestado)
			echo " selected=\"selected\"";
		...
		foreach ($lista_estados as $estado) {
			echo "<option value=\"".$estado["cod_estado"]."\"";
			if ($estado["cod_estado"] == $cadbo->getValorCampo("codestado_autor"))
				echo " selected=\"selected\"";
			echo ">".$estado["sigla"]."</option>\n";
		}
		?>
        ...
        <label for="selectcidade2">Cidade:</label><br />
        <select name="codcidade_autor" id="selectcidade2" class="slc<?=$cadbo->verificaErroCampo("cidade_autor")?>">
			<option>.</option>
		</select>
		<input type="text" class="txt<?=$cadbo->verificaErroCampo("cidade_autor")?>" style="display:none;" name="cidade_autor" id="cidade2" size="45" value="<?=htmlentities(stripslashes($cadbo->getValorCampo("cidade_autor")))?>" maxlength="100" /><br />

		<label for="fone-autor">Telefone:</label>
        <em><small>(Opcional) (Ex. (81) 3222-3333)</small></em> <br />
		<input type="text" class="txt small fone" id="telefone" name="fone_autor" value="<?=$cadbo->getValorCampo('fone_autor')?>" />
...
<br />
<div class="caption">
  <input type="checkbox" value="1" id="acordo" name="deacordo_autor"<?=($cadbo->getValorCampo('deacordo_autor'))?' checked="checked"':''?> />
...
obterCidades2(document.getElementById("estado2"), <?=(int)$cadbo->getValorCampo('codcidade_autor')?>);
</script>
<?php
