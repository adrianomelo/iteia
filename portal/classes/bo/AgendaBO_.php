...
class AgendaBO {
	...
	private $linkagenda = 'eventos.php';
	...
	public function getHtmlCalendario($mes, $dia) {
		...
		$texto_html .= "<thead><tr>\n";
		$texto_html .= "<th scope=\"col\">D</th>\n";
		$texto_html .= "<th scope=\"col\">S</th>\n";
		$texto_html .= "<th scope=\"col\">T</th>\n";
		$texto_html .= "<th scope=\"col\">Q</th>\n";
		$texto_html .= "<th scope=\"col\">Q</th>\n";
		$texto_html .= "<th scope=\"col\">S</th>\n";
		$texto_html .= "<th scope=\"col\">S</th>\n";
		$texto_html .= "</tr>\n</thead>\n<tbody>\n";
		...
		for ($count = 0; $count < (6*7); $count++) {
			...
			if ($count < $firstdayarray["wday"] || $dayarray["mon"] != $month)
				...
				$texto_html .= "<td $tdc  >".$texto_dia."</td>\n";
	...
	private function getTagsConteudo($codconteudo) {
		...
		foreach($this->contdao->getTagsConteudoNovo($codconteudo) as $tag)
			$html .= (($html != '') ? ' ' : ' ').'<a href="/busca_resultado.php?buscar=1&amp;tag='.urlencode($tag['tag']).'" class="common0 size0">'.$tag['tag'].'</a>';
		return $html;
	}
